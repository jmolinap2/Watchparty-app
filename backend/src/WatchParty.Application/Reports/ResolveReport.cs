...
public sealed record ResolveReportCommand(Guid AdminUserId, Guid ReportId, string? Note) : ICommand<Result>;
    ...
    : ICommandHandler<ResolveReportCommand, Result>
{
    public async Task<Result> Handle(ResolveReportCommand command, CancellationToken cancellationToken)
    {
        ...
        var result = report.Resolve(command.AdminUserId, command.Note);
        if (result.IsFailure)
        {
            return result.Error;
        }
        ...
        await auditLogRepository.AddAsync(
            AuditLog.Admin("report_resolved", command.AdminUserId, "Report", report.Id.ToString(), command.Note),
            cancellationToken);
